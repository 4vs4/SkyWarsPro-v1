...
class MapReset {
    ...
    public function saveMap(Level $level) {
        if(!file_exists($this->plugin->plugin->getServer()->getDataPath() . "worlds" . DIRECTORY_SEPARATOR . $level->getFolderName())) {
            return;
        }
        $level->save(true);
    ...
    public function loadMap(string $folderName): ?Level {
        ...
        if(!file_exists($zipPath)) {
            SkyWars::getInstance()->getLogger()->critical("Couldn't reload level {$folderName} (map archive was not found).");
            return null;
        }
